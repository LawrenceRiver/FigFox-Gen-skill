...
核心原则是**先规划、再图像生成、最后 SVG 校正**：可并行获取的证据同时进行，随后在图像模型出首稿前冻结唯一的拓扑合同与配色合同；一次可编辑 SVG 校正锁定结构层并渲染最终 PNG。
...
| 一次 SVG 校正 | 只检查一次文字、碰撞、箭头、层级、密度、几何和配色使用；随后用一份可编辑 SVG 锁定结构框、箭头、文字与纯色色块，保留复杂 raster 科研资产，并将 SVG 直接渲染为最终 PNG。不再进行第二次图像生成，也不能推翻重画。 |
...
我发现生成前的 SVG 或结构描述本身并不会自动让图像生成模型真正理解这张图。图像模型适合生成复杂科学资产和视觉形态，却常会改错文字、漂移颜色，也会放松人工科研图的几何纪律。因此，Skill 让它先生成首稿，再用一次 SVG 校正锁定几何、文字、连线、边距与纯色色块，同时保留复杂 raster 科研资产。

因此，这个 Skill 把关键控制前置：领域绘图惯例、语义–结构 FigureBench RAG、隔离的配色合同、精确文字和拓扑合同，然后才直接调用图像生成模型，并在首稿后进行一次 SVG 校正。目标不是泛化的“AI 图”，而是一张原创、可读、符合严肃科研论文视觉纪律的图。生成结果在作为科学证据使用前仍必须由作者独立核验。
