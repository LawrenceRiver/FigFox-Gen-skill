...
**科研论文图生成 · 网页领域绘图惯例 · FigureBench RAG · 冻结配色 · PNG→语义 SVG→图像生成**
...
核心原则是**先规划、再图像生成、再语义 SVG、最后图像生成**：可并行获取的证据同时进行，随后在图像模型出首稿前冻结唯一的拓扑合同与配色合同；VLM 根据首稿实际内容重构出可编辑、可简化的 SVG，再由该 SVG 约束一次最终图像生成并锁定文字与结构层。
...
| PNG→语义 SVG→最终图像 | 只检查一次文字、碰撞、箭头、层级、密度、几何和配色使用；VLM 依据实际 PNG 语义重构可编辑 SVG，而非像素描边，筛掉渐变、透视、畸形几何和无意义装饰。该 SVG 作为第二次图像生成的不可变结构参考，并以 SVG 覆盖层锁定最终 PNG 的精确文字与几何。 |
...
我发现生成前的 SVG 或结构描述本身并不会自动让图像生成模型真正理解这张图。图像模型适合生成复杂科学资产和视觉形态，却常会改错文字、漂移颜色，也会放松人工科研图的几何纪律。因此，Skill 先让它生成首稿，再让 VLM 根据首稿真实内容语义重构出更干净、可编辑的 SVG：保留核心模块、箭头、文字和确有科学意义的资产，去除渐变、透视、畸形容器和空洞装饰。这个 SVG 约束第二次图像生成，并在最终 PNG 上锁定文字与几何。

因此，这个 Skill 把关键控制前置：领域绘图惯例、语义–结构 FigureBench RAG、隔离的配色合同、精确文字和拓扑合同，然后才直接调用图像生成模型，并在首稿后进行一次语义 PNG→SVG 重构，再做最终图像生成。目标不是泛化的“AI 图”，而是一张原创、可读、符合严肃科研论文视觉纪律的图。生成结果在作为科学证据使用前仍必须由作者独立核验。
