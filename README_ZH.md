...
**领域共识视觉 + FigureBench RAG · 审美配色 · 拓扑优先的图像生成**
...
它不替代最终图像模型，而是先把科研图应有的科学关系、绘图惯例、审美配色和图像生成合同交给图像模型，再进行最终渲染。**先规划、再图像生成**：不会先渲染整张 SVG 再把它喂给图像模型。
...
| 图像生成合同 | 以结构化上下文锁定命名模块、相对布局、箭头关系、精确文字、参考裁图、FigureBench 摘要和 HEX 颜色角色，直接交给图像模型。 |
| 直接图像生成 + 一次检查 | 图像模型直接根据合同生成；只检查一次输出光栅图，并且只允许一次局部图像修补，不得重做整图。 |

## 实跑证据

工作流程图只有在它们以记录的 Figure Brief 为输入、完整产出参考记录、颜色合同、图像生成合同、图像模型直出、一次局部修补和 PNG 后，才会发布在这里。此前手工绘制或 SVG-first 的流程图已经移除，不会被作为实跑证据展示。
